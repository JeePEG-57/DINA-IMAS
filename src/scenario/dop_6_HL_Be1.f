...
c$
        ...
        if(k_ener.eq.1.and.ntay.ge.1)	then
...
c************************************
!	if(ntay.le.1.and.kefit.eq.5)vs=vs_start
c************************************
...
!           call zrad1(nz_imp,3,1,te_zrad,RES)
       ...
       if(kpr.eq.1)print*,'z_1 tec ',z_1,tec

c        coef_imp=(zeff_a-1.)/(z_2-zeff_a*z_1)
        coef_imp=(zeff_a-1.)/(z_2-z_1)
cccc    Nz we are calculating as >>>>>>   Nz=coef_imp*pcch
...
c        if(tt.gt.410000.)then
        if(ntay.gt.5.and.tt.gt.tt_dw)then
           key_help=1
           key_h_to_l=1
        end if
