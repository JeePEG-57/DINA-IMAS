...
c	call pll_new()
	...
	if(i_ramp.eq.1)call ind_ind()

	pshalo=del_r*dr_h
c	if(del_r.ge.50.)i_del=i_del+1
	if(i_del.eq.1)then
	pshalo0=pshalo
        w_h0=pshalo0/delaval0
	end if

c--- NEW LINES ....>>>

        if(ntay.eq.10)delaval0=delaval
        pshalo0=w_h0*delaval0
        if(kpr.eq.1)print *,' pshalo pshalo0',pshalo,pshalo0

        if(pshalo.ge.pshalo0.and.ntay.gt.11)k_wh=1

	if(k_wh.eq.1)then
	if(kpr.eq.1)print *,' k_wh=========',k_wh
	pshalo=pshalo0
	del_r=pshalo/dr_h
	end if
...
c----------------------------------------<<<<
	...
	p_s=pbound-pshalo
	w_h=(pbound-p_s)/delaval
	if(ntay.gt.10)w_h=(pbound-p_s)/delaval0

        if(kpr.eq.1)print *,' w_h w_h0',w_h,w_h0
c------------------------------------
c	ipage=1
c	if(i_graph.eq.0)ipage=0
c----------
c	yyy=-1.e10
c	do i=1,jbound
c	   if(ybound(i).gt.yyy)then
c	      yyy=ybound(i)
c	      xxx=xbound(i)
c	   end if
c	end do
c	if(kpr.eq.1)print*,'xxx yyy',xxx,yyy
c	pause 'from ptoke1'
...
c--------------------
	...
	if(kpr.eq.1)
     *	print *,' del_r pshalo p_s dr_h',del_r,pshalo,p_s,dr_h
...
71	format(20x,a6/,(6(1pe10.3)))
	...
	if(kpr.eq.1)print *,' rmag zmag zref',rmag,zmag,zref
...
c ----->
	...
	if(k.eq.ind_z(1))psext0=psext0+x2*FLUXARR(kk,K)
...
c	if(kpr.eq.1)print*,'dx dy coef',dx,dy,coef
c	if(kpr.eq.1)print*,'psi_pf tpl tok_test',psi_pf,tpl,tok_test
c
	tokc=0.
	DO K=1,ncam
	tokc=tokc+tcam(K)
...
c	if(kpr.eq.1)print *,'***psepa gt psep---',psepa,psep
	...
	zsep=zsepa
      ...
      if(kpr.eq.1)print *,' i_min dist_min==',i_min,dist_min
      ...
      n_div=500
      if(kpr.eq.1)print *,' i1 i2 ke n_div==',i1,i2,ke,n_div

      if(kpr.eq.1)print *,' rsep zsep psep==',rsep,zsep,psep
...
c
!	if(kpr.eq.1)print *,' xx zz ival jval===',xx,zz,ival,jval
...
!      if(kpr.eq.1)print *,' i xu yu =',i,xu(i),xu(i-1),yu(i),yu(i-1)

      do j=1,n_div
      urr=xu(i-1)+dxx*float(j)
      vrr=yu(i-1)+dyy*float(j)

!      if(kpr.eq.1)print *,' i j urr vrr=dxx dyy =',i,j,urr,vrr,dxx,dyy

	call boxd(urr,vrr,pdd,ier)
	p_lim=pdd(1)

!      if(kpr.eq.1)print *,' i j p_lim psep==',i,j,p_lim,psep
...
c
	...
	n_cc=n_cc+1
 	...
 	fpl=fpl+0.1d0*F(kk)*vesarr(kk,k) 	
 	end if
	...
	if(kpr.eq.1)print *,' psp k n_cc=',psp(k),k,n_cc
...
c	hpart=1.05
	...
	al1=tpl/(tok+tok_g)
C
	if(kpr.eq.1)print *,'tpl hpart npoint',tpl,hpart,npoint
...
c	write (44)(pspl0(i),i=1,nwnh)
	...
	if(kpr.eq.1)print *,' end of writing for044 file'
	close (unit=44)
	end if
...
c	read (44)(pspl0(i),i=1,nwnh)
	...
	if(kpr.eq.1)print *,' end of reading for043 file'
	close (unit=44)
	end if
c
	return
...
c
	...
	if(kpr.eq.1)print *,' print from f40'
...
c	if(kpr.eq.1)print*,'npf npf_help',npf,npf_help
c	read(*,*)
...
c	end do
c	if(kpr.eq.1)print*,'!!! tpl tok_test coef',tpl,tok_test,coef
c	read(*,*)
...
c	write (6,*)(pffx(i),i=1,nrad)
	...
	do i=1,np
	   ...
	   psix=(dfloat(i)-1.d0)/dfloat(n1)
	   	   
	   if(psix.le.1.d0)then
	   call feeti(nrad,ppx,pprime,poa,psix)
	   call feeti(nrad,pffx,fprime,poa,psix)	 
...
c
	...
	END DO
        ...
        if(kpr.eq.1)print *,' tokc  eu ===',tokc,eu
...
c	if(kpr.eq.1)print *,' i j dist index_ves ',i,j,dist,index_ves(kk)

c	if(index_ves(kk).eq.1.and.dist.le.eu)then
	if(dist.le.eu)then

c	if(kpr.eq.1)print *,' i j psix pspl ',i,j,psix,pspl(kk)
...
c	eu=sqrt( abs(sp_pl)/(pi*elong) )
	...
	if(eu.lt.1.)eu=1.
        ...
        if(kpr.eq.1)print *,' sp_t sp_pl elong EU=====',sp_t,sp_pl,elong,eu
...
c       
	...
	if(kpr.eq.1)print *,' n_sep kk k_d psep ==',n_sep,kk,k_d,psep
...
c                                                                       
	...
	if(kpr.eq.1)print *,' aval psep2 ==',aval,psep2
	      
	if(dabs(psep2).gt.5.d11)then
	rsep2=0.d0
	zsep2=0.d0
	n_sep2=0
	if(kpr.eq.1)print *,' n_sep2 rsep2 zsep2 ==',n_sep2,rsep2,zsep2
	end if
...
c       
	...
	if(kpr.eq.1)print *,' n_sep2 kk  psep2 ==',n_sep2,kk,psep2
...
c	if(kpr.eq.1)print *,' rm zm nxb ntet pi',rm,zm,nxb,ntet,pi
...
c     *  ig,tetxb(ig),tetp_in,tetp_in2,drx,deltet
 ...
 200        continue
	...
	apr='roxb'                                                              
c	if(kpr.eq.1)print 71,apr,(roxb(j),j=1,nxb) 
...
c	if(kpr.eq.1)print *,' brad bvert ',brad,bvert
...
c-------------------
	...
	num=20
c	call out42(n_pr,a_print,num,apr)
c        if(kpr.eq.1)print *,' brad_fin bvert_fin ',brad_fin,bvert_fin
...
c	if(kpr.eq.1)print*,'from write_separ_coor'
	...
	if(kpr.eq.1)print*,'!!!tt ksepa',tt,ksepa
...
c	if(kpr.eq.1)print *,' rsep2 zsep2 psep2 ksep',
...
!	ke=ke1
      ...
      if(kpr.eq.1)print *,' ke1 ke===========',ke1,ke
            
      if(ke.gt.1999)then      
      if(kpr.eq.1)print *,' ke gt 2000',ke
      stop
      end if
...
!	yu(i)=yu(i)+70.d0
	...
	call boxd(urr,vrr,pdd,ier)
	psi_lim(i)=pdd(1)
!	 if(kpr.eq.1)print *,' i vrr psi_lim psep2',i,vrr,psi_lim(i),psep2
	else
!	 if(kpr.eq.1)print *,' --- i urr vrr',i,urr,vrr
	end if
...
!	if(kpr.eq.1)print *,' --- i ps_1 psep2 ps_2  vrr',i,ps_1,psep2,ps_2,vrr
...
!	if(kpr.eq.1)print *,' --- ps_1 ps_2 psep_2 psep2',ps_1,ps_2,psep_2,psep2
...
!	if(kpr.eq.1)print *,' +++i_int psep_2 psep2 pmag',i_int,psep_2,psep2,pmag

      if(vrr.gt.0.and.kpr.eq.1)then
...
!	print *,' +++vrr1 vrr vrr2',yu(i-1),vrr,yu(i) 
	...
	r_lim(k)=urr
	z_lim(k)=vrr
	 if(kpr.eq.1)print *,' +++ k r z',r_lim(k),z_lim(k)
	n_lim=k
	end if
...
2     continue
	 ...
	 if(kpr.eq.1)print *,' rsep2 zsep2 n_lim',rsep2,zsep2,n_lim
!	 if(kpr.eq.1)print *,' r_min r_max z_max',r_min,r_max,z_max
...
c*************** Victor's additional *************************
	...
	if(ksepa.eq.1)then
	 ...
	 if(kpr.eq.1)print *,' rsep2_r zsep2_r',rsep2_r,zsep2_r
	 
!	if(kpr.eq.1)print*,'ksepa rsep2_gr zsep2_gr n_lim',
