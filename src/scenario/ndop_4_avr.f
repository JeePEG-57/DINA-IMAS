...
c	end if
	...
	xp8(k)=xp(k)
c	if(jpr.eq.1.and.kpr.eq.1)print *,'k i xp8',k,i,xp8(k)
	return
	end if
c
	if( i.eq.1) then
...
c	if(kpr.eq.1)print 71,apr,(psval(i),i=1,n)                               
  	apr='-poax-'                                                            
c 	if(kpr.eq.1)print 71,apr,(poax(i),i=1,n)                               
...
!	errp=dmax1(err,errp) 
	if(err.gt.errp)then
c	if(kpr.eq.1)print *,' i j err y ',i,j,err,ypl(i,j),y_map(i,j)
	errp=err
	end if
...
!	errp=dmax1(err,errp) 
	if(err.gt.errp)then
c	if(kpr.eq.1)print *,' i j err ',i,j,err
	errp=err
	end if
