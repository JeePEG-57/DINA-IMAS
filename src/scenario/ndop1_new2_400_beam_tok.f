...
c *** to do PTOKE
	...
	apr='ppz'
	if(kpr.eq.1)print 71,apr,(ppz(i),i=1,nrad)
	apr='pffz'
...
!	call feeti(n,ppz,ppx(i),aiz,a(i))
!	call feeti(n,pffz,pffx(i),aiz,a(i))
...
71	format(20x,a6/,(6(1pe10.3)))
	...
	pffx(1)=pffx(2)
 	...
 	ateta=1.d0
        call inter_h0(ppx,a,n-1,ateta,val)
	ppx(n)=val
	ppx(n)=ppx(n-1)
	
 	ateta=1.d0
        call inter_h0(pffx,a,n-1,ateta,val)
	pffx(n)=val
      pffx(n)=pffx(n-1)
...
c+++++++++++++++++++++++++++++++++
	...
	p(i)=( pne(i)*te0(i)+( pd0(i)+pt0(i)+ppr(i))*tq0(i) )*200.*1.e-6

!	p(i)=( pne(i)*te0(i)+( pne(i)+ppr(i) )*tq0(i) )*200.*1.e-6
...
!	p(i)=( pne(i)*te0(i)+( pne(i)+ppr(i) )*tq0(i) )*200.*1.e-6
	...
	pp(i)=-(p(i)-p(i-1))/(ha(i)*psi(i))*2.*pi*rs0*0.01
...
c
	...
	apr='-psi-'                                                            
	if(kpr.eq.1)print 71,apr,(psi(i),i=1,6)
...
c	print 71,apr,(dfmaxc(i),i=1,n) 
	...
	if(errp.gt.1.e-5)go to 1                                                
	if(err.gt.1.e-5)go to 1                                                
...
c	pffx(i)=coef*(pffx(i)+pffx1(i)+pffx2(i))
	...
	subroutine avr_ppx_pffx2()
      include 'double.inc'
	include 'new_com.inc'

	call avr_ppx_pffx2_c(
     *  n,ppx,pffx,ppx1,pffx1,ppx2,pffx2,omg_ppx,kpr)


	return
	end

	subroutine avr_ppx_pffx2_c(
     *  n,ppx,pffx,ppx1,pffx1,ppx2,pffx2,omg_ppx,kpr)
...
71	FORMAT(5X,A10/,(2x,6(1PE11.3)))
      ...
      if(kpr.eq.1)print *,' omg_ppx==',omg_ppx

	do i=1,n
	ppx(i)=(1.d0-omg_ppx)*ppx(i)+omg_ppx*ppx1(i)
	pffx(i)=(1.d0-omg_ppx)*pffx(i)+omg_ppx*pffx1(i)
	end do
...
!	c_pol=0.9
	...
	subroutine ppx_pffx_corr4()
	include 'double.inc'
c	implicit real *8 (a-h,o-z)
	include 'new_com.inc'

         call ppx_pffx_corr4_c(
     *   rs0,
     *   n,ppx,pffx,a,pi,kpr)
...
c	c_pol=0.75
	...
	subroutine ppx_pffx_corr3()
	include 'double.inc'
c	implicit real *8 (a-h,o-z)
	include 'new_com.inc'

         call ppx_pffx_corr3_c(
     *   rs0,
     *   n,ppx,pffx,a,pi,kpr)
...
71	FORMAT(5X,A10/,(2x,6(1PE12.5)))
	...
	if(kpr.eq.1)print *,' kpr from pet_tab==',kpr
...
c	   call feeti(nutab,fptab,pffx(i),pstab,psn)
     ...
     *  /halo11/fmaxv,fmaxv0                                            
      ...
      common     
     *  /vic_uli/del_li,uli_p,ch_k,ulivel,pcchp0
...
71	FORMAT(20X,A8/,(6(1X,1PE10.3)))                                      
	...
	CALL BTA(n,mp,RS0)
      uli_p=uli
      if(kpr.eq.1)print *,'uli_t uli_p ',uli_t,uli_p
