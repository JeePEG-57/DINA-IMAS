...
c      if(kpr.eq.1)print*,'!!! ntay=',ntay
      MTE='TE0'
c      IF(KPR.EQ.1)PRINT 71,MTE,(TE0(i),i=1,n)
c      pause 'from zu0'
...
c
   ...
   21 CONTINUE
      MFE='FE'
      MFQ='FQ'
C     IF(KPR.EQ.1)PRINT 71,MFE,(FF(I,1),I=1,N)
C     IF(KPR.EQ.1)PRINT 71,MFQ,(FF(I,2),I=1,N)
      TT(N,1)=TE0(N)
      TT(N,2)=TQ0(N)
...
c
      DO 6 I=2,N
        ...
        if(TT(i,1).lt.10.)TT(i,1)=10.
        if(TT(i,2).lt.10.)Tt(i,2)=10.
...
c      IF(KPR.EQ.1)PRINT 71,MTE,(TE0(i),i=1,n)
      MTE='PNE'
c      IF(KPR.EQ.1)PRINT 71,MTE,(pne(i),i=1,n)
c      pause 'from energy'
c      if(kpr.eq.1)print*,' **nn  **     ***',nn
      MTQ='TQ0'
c      IF(KPR.EQ.1)PRINT 71,MTQ,(TQ0(i),i=1,n)
      NN=NN+1
...
c       implicit real*8 (a-h,o-z)
	...
	src=src+sd0(i)+st0(i)
        DIF(I)=0.4*XII(I)
    1 CONTINUE
ccc	if(kpr.eq.1)print *,' source============================',src
ccc	pause
   71 FORMAT(20X,A6/,(8E10.3))
      RETURN
      END
