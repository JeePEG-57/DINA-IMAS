...
c        if(kpr.eq.1)print*,'bz_psi br_psi',bz_psi,br_psi
c        if(kpr.eq.1)print*,'bz br',bz,br
...
c**********************************************
	...
	call vic_dens_c(
     *       pcchp,eu,tpl,pi,t_end,tt,tt_dw,ntay)
...
c	if(ntay.gt.30.and.tt.gt.tt_dw+4.e3.and.key_gamma.eq.0)then
	if(ntay.gt.30.and.tt.gt.tt_dw+dt_1.and.key_gamma.eq.0)then
	   key_gamma=1
...
c        if(kpr.eq.1)print*, 'from victor.f'
c	read(*,*)
...
c
	...
	if(kpr.eq.1)print*,'next ntay tpl cip1',next,ntay,tpl,cip1
...
c       end if
 ...
 1000	continue
c	if(kpr.eq.1)print*,'n_coil_xx bp',n_coil_xx,bp
c	read(*,*)
...
c	do i=1,8
c	   if(kpr.eq.1)print*,'r_surf(i) z_surf(i)',r_surf(i),z_surf(i)
c	   end do
c	read(*,*)
...
c*********************************************************
	...
	if(kpr.eq.1)print*,'1from vic_feed_aux'
	if(kpr.eq.1)print*,'emoe emoq betpj tt',emoe,emoq,betpj,tt
...
c	betp_end=0.4
	...
	if(tt.ge.500000..and.kpr.eq.1)then
	   print*,'from vic_feed_aux'
	   print*,'tt tt_dw dt',tt,tt_dw,dt
	   print*,'betp_flat betp_beg betp_end',
     *     betp_flat,betp_beg,betp_end
	end if
...
c Kavin's insert, tay should be equal time step
           Tfilt_emo=0.6
           del_emo=(-alfa_p*tay*1.e-3*(betpj-betp_flat)+
     *     Tfilt_emo*del_emo_0)/(Tfilt_emo+tay*1.e-3)
...
c end Kavin's insert
	...
	if(kpr.eq.1)then
	print*,'2from vic_feed_aux'
	print*,'betpj betp_flat',betpj,betp_flat
	print*,'deriv del_emo',deriv,del_emo
	print*,'emoe emoq',emoe,emoq
c	read(*,*)
	end if
...
c
	...
	if(kpr.eq.1)print *,' s_plasma==',s_plasma
