        subroutine right1(a_s,b_s,c_s,d_s,ui)
...
c         include'double.inc'
         ...
         dimension  ui(nip,njp)

         dimension a_s(nip,njp),b_s(nip,njp),c_s(nip,njp),
     *  d_s(nip,njp)
...
C        C(i,j)*U(i,j+1)+D(i,j)*U(i,j-1)+E(i,j)*U(i,j)=F(i,j)
           ...
           right(il)=right(il)-d_s(i,j)*ui(i,j-1)
...
C        C(i,j)*U(i,j+1)+D(i,j)*U(i,j-1)+E(i,j)*U(i,j)=F(i,j)
           ...
           right(il)=right(il)-c_s(i,j)*ui(i,j+1)
...
C        C(i,j)*U(i,j+1)+D(i,j)*U(i,j-1)+E(i,j)*U(i,j)=F(i,j)
           ...
           right(il)=right(il)-a_s(i,j)*ui(i+1,j)
...
C        C(i,j)*U(i,j+1)+D(i,j)*U(i,j-1)+E(i,j)*U(i,j)=F(i,j)
           ...
           right(il)=right(il)-b_s(i,j)*ui(i-1,j)
...
c        print *,' ni1 nj1 ni2 nj2 neq==',ni1,nj1,ni2,nj2,neq
 ...
 950    continue
        ...
        return
         ...
         subroutine matrix(a_s,b_s,c_s,d_s,e_s,nr,nz)
...
c        write(6,*) 'matrix:neq,nnz',neq,nnz
c        write(6,*) 'matrix:neqp,lp',neqp,lp
...
c        equivalence (right(1),zw(1))
         ...
         path=1
         call odrvd(neq,ia,ja,a,p,ip,nspp,isp,path,flag)
...
c         call PDRVD
c     *        (Neq, IA,JA,A, P,IP, NSPp,ISP, PATH, FLAG)
...
c         call CDRVD
c     *     (Neq, p,pp,Ip, IA,JA,A, right, Zw, NSPp,ISP,
c     *  RSP,ESP, PATH, FLAG)
...
c         write(6,*) '***il right znev znmx',il,right(il),znev,znmx
