function [sys,x0,str,ts] = simimasdialog(t,x,u,flag)
...
wkf='/home/ITER/huynhp/ITERIS/TEST/test3.xml'
%[s,t]=unix(sprintf('/home/ITER/huynhp/ITERIS/TEST/launch_IMAS.sh %s',wkf));
%t
%s=unix(sprintf('nohup /home/ITER/huynhp/ITERIS/TEST/launch_IMAS.sh %s > /dev/null',wkf));
%s=unix(sprintf('nohup toto.sh %s > /dev/null',wkf));
s=unix(sprintf('nohup  /home/ITER/huynhp/ITERIS/TEST/launch_KEP.sh %s > /dev/null',wkf));
s=unix(sprintf('nohup /home/ITER/huynhp/ITERIS/TEST/get_KepID.sh %s > /dev/null',wkf));
%le fichier KEPID contient les PID des processus kepler 
fileID=textread('/home/ITER/huynhp/ITERIS/TEST/KEPID','%s','delimiter','\n','whitespace','');
...
if SimFlag == 0
    [sreturn,treturn]=unix(sprintf('/home/ITER/huynhp/ITERIS/TEST/activate_IMAS.sh %s',fileID{1}));
    [sreturn,treturn]=unix(sprintf('/home/ITER/huynhp/ITERIS/TEST/activate_IMAS.sh %s',fileID{1}));
    treturn
    %return the mdl name
    MDLNAME=gcs;
    sprintf('/home/ITER/huynhp/ITERIS/TEST/focus_SIM.sh %s',MDLNAME);
    %[s,t]=unix(sprintf('/home/ITER/huynhp/ITERIS/TEST/focus_SIM.sh %s',MDLNAME));
