...
C
              W = W*DFM - 0.5*H*(ALOG(DFJ/DTJ)*DFJ+ALOG(DFJ1/DTJ1)*DFJ1)
C
           ELSE
C
...
c	coef_pet=2.5
	...
	zzk=zp
C
      GREENI =fp_b(rr,rrk,zz,zzk)/coef_pet

c	if(kpr.eq.1)print *,' greeni===',greeni
C
      RETURN
      END


      FUNCTION FP_bint(R,R1,Z,Z1)
	include 'double.inc'
c       implicit real*8 (a-h,o-z)
	common
     *  /ge1/pi
...
c	r_2=r+0.5d0*dist0
	...
	call bint(r1,z1,r_1,z_1,r_2,z_2,Fint_c,2)
	Fint_c=coef_pet*Fint_c*pi
