...
!	include 'for/const.inc'
	...
	double precision YDABL,YDDEP,YVOL,yswitch,YPELSRS(*)
c======================================================================
...
!         write(*,*) 'YAM,YVP,YVOL,YCOS0,YEFF,YDL,YDABL,YDDEP,yswitch'
!         write(*,*) YAM,YVP,YVOL,YCOS0,YEFF,YDL,YDABL,YDDEP,yswitch
!	write(*,*) 'ne',ne(1:na1)
!	write(*,*) 'Te',Te(1:na1)
!	write(*,*) 'ni',ni(1:na1)
!	write(*,*) 'nhydr',nhydr(1:na1)
!	write(*,*) 'ndeut',ndeut(1:na1)
!	write(*,*) 'ntrit',ntrit(1:na1)
!	write(*,*) 'ametr',ametr(1:na1)
!	write(*,*) 'Ti',Ti(1:na1)
!
!         write(*,*) 'HRO,ROC',HRO,ROC 
...
!        write(*,*) 'NA,JABS,na1=',NA,JABS,na1
	...
	do	J=NA1,JABS,-1
		...
		JBEG=JDEL/2
		JBEG=J+JBEG
	if(JDEL.le.0) write(*,*) 'j,JDEL,YKCR(J)=',j,JDEL,YKCR(J)
...
CVOLUM(J)-VOLUM(J-1)
		...
		YDNE=YDNE+DNI(JJ)
	enddo
		if(ydv.le.0.) write(*,*) 'j,YDV,HRO =',j,YDV,HRO
	YDNI = YDNE/YDV
	YNE	=NE(J)+YDNI*Zp
	YNI	=NI(J)+YDNI
		if(yni*yne.le.0.) 
     .  write(*,*) 'j,NE(j),NI(j),YNI,YNE =',j,NE(j),NI(j),YNI,YNE
...
c*19-NOV-2013 vvvvvvvvvvvvvvvvvv
	 if(yswitch.gt.0.d0) then	!=================== start switch
...
C======================================================================|
		subroutine	SMTH(ALFA,NO,FO,XO,N,FN,XN)
C----------------------------------------------------------------------|
