...
! solsrsN_IMAS is made from solsrs_IMAS. 
! pellet injection parameters are arranged as input variable
! control of fuelling and ELM pacing is removed. (commented by !!)
...
! Extra input variables to be controlled externally: 
!	yVpelH	[mm3]		HFS pellet volume (was precet =33.d0)
!	yVpelL	[mm3]		LFS pellet volume (was precet =33.d0)
!	yfHFS	[Hz]		HFS pellet frequency
!	yfLFS	[Hz]		LFS pellet frequency
!	yCpel	[km/s]		pellet speed
!	yMpel	[a.u.]		Mpel/Mp (1, 2-3)
!	yCOST	[cosT]		cosine of the angle to normal of external magn. surf.(negative for LFS)
!	yDL	[a.u.]		dL/a = 1/cosAlf (Alf = angle between injection and midplane
! OUT:
...
!	ySHe	[10^19/s/m3]	voulmetric source from the edge He neutrals
!	yNNHe	[10^19/s/m3]	neutral He density distribution
...
!	include 'for/status.inc'
	...
	character*17 filnam
	real*8
     .		yELONG,yABC,yTauE,yTaup,yCoef,yTAUOH,yRTOR,yL_OH,yMod
      real *8
     . yCpel,yMpel,yCOST,yDL,yfLFS,yfHLS,ySHep,yNNHe
...
!========================================
	...
	data yGpel/0.d0/,ypn/0.d0/,ydndt/0.d0/ 
...
!!!		yGHFS	=yfHFS*yNpelH	!!! NEW	
		yfHFS	= yGHFS/yNpelH
		
		yGLFS	=yfLFS*yNpelL	!!! NEW
		yfHLS=0.
		yfpel 	=yfHFS+yfHLS	!!! NEW
...
!!!		yGHFS	= max((yGELM+yGsol-yGsep-yGNBI),.0d0)
		if(yfpel.gt.0.) then
		 if((yALP*yPsol/yfpel/yDWELM).gt.1.)write(*,*)
     .        'ELM size is above the limit DWELM/DWELM,max = ',
     .		yALP*yPsol/yfpel/yDWELM
		endif
	
		if(yGHFS.gt.(216.*54.)) then
!!!		yGHFS=216.*54.
!!!		write(*,*) '
!!!     .  'required pellet fuelling exceeds the design maximum Gpel=216 PAm3/s ',
!!!     .  'pellet fuelling was set at maximum GHFS=216 PAm3/s'
	write(*,*) 'required pellet fuelling exceed the design limit', 
     .  'GHFS =',yGHFS,'>Gpel=216 PAm3/s',
     .  ' reduction of fHFS and/or VpelH is required'	!!!
		endif
...
!	write(*,*) 'sP--> yGELM,yGHFS,yGsol,yGsep,yfHS= ',
...
!	write(*,*) 'before pelsrs1_IMAS'
	call pelsrs1_IMAS
     .	(yMpel,yCpel,YVpelH,yCosT,1.d0,yDL,YDABL,YDDEP,ySpel,yswitch)
!!!     .	(1.d0,0.3d0,YVpelH,0.8d0,1.d0,1.67d0,YDABL,YDDEP,ySpel,yswitch)

	write(*,*) 'sP--> 1.d0,0.3d0,YVpelH,0.8d0,1.67d0= ',
     .	yMpel,yCpel,YVpelH,yCosT,1.d0,yDL


		ySRS0	=VINT(ySpel,ROC)
	
!	write(*,*) ' sP-ySpel', (ySpel(j),j=1,na1)
...
!		write(*,*) 'after solpsz2'
	if(yMu.gt.1.d0) write(*,*)'detachment! mu =',yMU
!==============================================
!       edge density can be reduced by increase of impurity Ynim (< 0.02)
!===========================================================
!	call pufhe(yGHe,yTi,yTe,yShep,yNNHe)	! Neutral He 
!==================================
