...
if (.not. associated(core_sources%source))  allocate(core_sources%source(1))    
if (.not. associated(core_sources%source(1)%profiles_1d))  allocate(core_sources%source(1)%profiles_1d(1)) 
 
if (.not. associated(core_sources%source(1)%profiles_1d(1)%grid%rho_tor_norm))  allocate(core_sources%source(1)%profiles_1d(1)%grid%rho_tor_norm(n))
...
allocate(core_sources%source(1)%profiles_1d(1)%electrons%particles(n))
 core_sources%source(1)%profiles_1d(1)%electrons%particles(1:n) = pne(1:n)*1.d19

!if (.not. allocated(core_sources%source(1)%profiles_1d(1)%ion)) then
...
! if (.not. allocated(core_sources%source(1)%profiles_1d(1)%ion(1)%n_i)) then
allocate(core_sources%source(1)%profiles_1d(1)%ion(1)%particles(n))
! end if
 core_sources%source(1)%profiles_1d(1)%ion(1)%particles(1:n) = pd0(1:n)*1.d19
...
allocate(core_sources%source(1)%profiles_1d(1)%ion(2)%particles(n))
 core_sources%source(1)%profiles_1d(1)%ion(2)%particles(1:n) = pt0(1:n)*1.d19
