...
c!!!      TE0(I)=TT(I,1)*0.75+tee(i)*0.25
   ...
   16 CONTINUE
      MTE='TEn'
      if(kpr.eq.1)PRINT 71,MTE,(TEn(i),i=1,n)
      MTE='TE0'
...
c*** Here Te must be in keV
	...
	return
      ...
      SUBROUTINE BSCOEFF2(pft,pq,pR,peps,pte,pne,pti,pni,pzeff,pzion, 
     * pl31,pl32,pl34,palfa,signeo)
...
!     finite nustar
      call neobscoeff2(pl31,pl32,pl34,palfa,pft,pzeff,znuestar,znuistar)

      call sigmaneo2(signeo,sigsptz,znuestar,pft,pne,pte,pzeff,
     * pq,pR,peps)
...
!
      ...
      SUBROUTINE neobscoeff2(L31, L32, L34, ALFA, ft, 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  !
  !
      SUBROUTINE sigmaneo2(signeo,sigsptz,nuestar,ft,ne,te,zeff,q,R,eps)
	include 'double.inc'
...
3     if(abs(f-y).lt.crit) iturn=1
   ...
   10 if((y-ya)*(y-yb).lt.0.) go to 1
      ...
      subroutine cubic2(p1,p2,p3,p4,p,a,b,c,d)
       implicit real *8 (a-h,o-z)
      data  zero/1.e-6/
      p1p2=p1-p2
      k12=sign(1.5d0,p1p2)
