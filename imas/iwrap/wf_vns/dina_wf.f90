...
do iloop=1,imax !DINA Loop
  ...
  ! First loop, put workflow and code parameters in the output IDS
  if (iloop.eq.1) then
  ...
  ! Check if time to switch on external transport
  ...
  ! Exit if time limit reached, plasma current lost, OR plasma volume collapsed (VDE/disruption)
  if (summary%time(1).gt.time_stop &
    .or. (dabs(summary%global_quantities%ip%value(1)).lt.1.d3 .and. current_pf_stop.lt.1.d3) &
    .or. error_flag.ne.0 &
    .or. equilibrium%time_slice(1)%global_quantities%volume.lt.1.d0) then
    write(*,*) 'Exit condition met: time=', summary%time(1), &
      ' ip=', summary%global_quantities%ip%value(1), &
      ' V=', equilibrium%time_slice(1)%global_quantities%volume, &
